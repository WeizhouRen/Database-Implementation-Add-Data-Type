...
char *extract_nums(char *str, int32 *strLen);
int32 *get_data(char *str, int32 *strLen, int32 *size);
char *to_string(int32 *data, int32 size, int32 strLen); 
...
intset_in(PG_FUNCTION_ARGS)
{
	...
	result = (IntSet *) palloc((size + 3) * sizeof(int32));
	SET_VARSIZE(result, (size + 3) * sizeof(int32));
...
intset_out(PG_FUNCTION_ARGS)
{
	...
	result = to_string(intSet->data, intSet->size, intSet->strLen);
...
int32 *get_data(char *str, int32 *strLen, int32 *size) {
	...
	int32 i = 0, j = strlen(str) - 1, n = 0, subLen = 0, subTokenLen = 0, numsLen = 0, num = 0, tmpSize = 0, *data = NULL, pos = 0;
	...
	// }
	if (tmpSize == 0) *strLen = 0;
	else *strLen = numsLen - 1;
	*size = tmpSize;
